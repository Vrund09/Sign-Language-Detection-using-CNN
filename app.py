...
# Functions for landmark processing
def calc_landmark_list(image, landmarks):
    image_width, image_height = image.shape[1], image.shape[0]
...
def pre_process_landmark(landmark_list):
    ...
    return [i / max_value for i in flat]

# Streamlit UI
st.set_page_config(page_title="Hand Signs", layout="wide")
st.title("🖐️ Real-time Hand Sign Recognition")

start = st.button("Start Camera")

frame_window = st.image([])
output_letter = st.empty()
output_sentence = st.text_area("Sentence", "")
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    sign_id = keypoint_classifier(processed)
                    cur = keypoint_classifier_labels[sign_id]

            # Display predicted letter
            if cur == prev:
                output_letter.markdown(f"<h1 style='text-align: center; color: green;'>{cur}</h1>", unsafe_allow_html=True)
            elif cur:
                prev = cur
